...
class verifyCode {
	 ...
	 * @var string
	 */
	public static $codeSet = '23467892ABCDEFGHJKLMPQRTUVWXY';
	...
	public static function entry() {
		...
		for ($i = 0; $i<self::$length; $i++) {
			...
			// 写一个验证码字符
			// 验证码使用随机字体 每个字不一样的字
			$ttf = dirname(__FILE__).'/fonts/' . mt_rand(1, 2) . '.ttf';
			imagettftext(self::$_image, self::$fontSize, mt_rand(-20, 20), $codeNX, self::$fontSize*1.5, self::$_color, $ttf, $code[$i]);
		}
		...
		Yii::app()->session[self::$seKey] = array('code'=>join('', $code),'time'=>time());
	...
	public static function check($code) {
		// 验证码不能为空
		if(empty($code) || empty(Yii::app()->session[self::$seKey])) {
			return false;
		}
		// session 过期
		$_ = Yii::app()->session->get(self::$seKey);
		if(time() - $_['time'] > self::$expire) {
			unset(Yii::app()->session[self::$seKey]);
			return false;
		}
		if( strtolower( $code ) == strtolower( $_['code'] )) {
			unset(Yii::app()->session[self::$seKey]);
			return true;
		}
		return false;
